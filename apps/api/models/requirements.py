...
class JiraTicketIntake(Base, TimestampMixin):
    ...
    planner_user_id: Mapped[UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"))
    assignee_user_id: Mapped[UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"))
    parent_jira_key: Mapped[str | None] = mapped_column(String(32))
    intake_mode: Mapped[str] = mapped_column(String(24), default="member", nullable=False)
    jira_project_key: Mapped[str | None] = mapped_column(String(32))
    jira_tasks_json: Mapped[dict | None] = mapped_column(JSONB)
